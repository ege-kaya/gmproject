...
def train_vae(model, train_dataloader, epochs, step_size):
    ...
    for epoch in range(epochs):
        ...
        plt.savefig("vae2_fake_images.png")                                 
        plt.show()    
        
        # Save the model and the loss values every 5 epochs.
        if epoch % 5 == 4:
            torch.save(model.state_dict(), f"models/vae2_{epoch+1}")
        
            with open(f"losses/vae2_losses_{epoch+1}", "wb") as fp:
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("models/vae2_18"))
