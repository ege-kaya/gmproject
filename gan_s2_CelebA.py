...
class Generator(nn.Module):
    def __init__(self):
        ...
        model = [nn.ConvTranspose2d(100, 1024, 3, stride=1, padding=0), #512 x 3 x 3
                 ...
                 nn.BatchNorm2d(512),
                 nn.ReLU(inplace=True),
                 nn.ConvTranspose2d(512, 256, 4, stride=2, padding=1), #256 x 5 x 5
                 nn.BatchNorm2d(256),
                 nn.ReLU(inplace=True),
                 nn.ConvTranspose2d(256, 128, 4, stride=2, padding=1), #256 x 20 x 20
                 nn.BatchNorm2d(128),
                 nn.ReLU(inplace=True),
                 nn.ConvTranspose2d(128, 64, 4, stride=2, padding=1), #128 x 40 x 40
                 nn.BatchNorm2d(64),
                 nn.ReLU(inplace=True),
                 nn.ConvTranspose2d(64, 1, 4, stride=2, padding=1), #1 x 80 x 80
                 nn.Tanh()
                 ]
...
class Critic(nn.Module):
    def __init__(self):
        ...
        model = [nn.Conv2d(1, 32, 7, padding=3),
                nn.BatchNorm2d(32, 0.8), # 32x64x64
                 nn.LeakyReLU(0.2, True), 
                #  nn.Dropout2d(0.25), 
                 GANBlock(32, 64),       # 64x32x32
                 GANBlock(64, 128),      # 128x16x16
                 GANBlock(128, 256),       # 256x8x8
                 GANBlock(256, 512)]      # 512x4x4
...
_, split2, _ = torch.utils.data.random_split(data, [100000, 100000, 2599])
    
if __name__ == "__main__":

    train_dataloader = torch.utils.data.DataLoader(split2, batch_size=32, num_workers=8, shuffle=True, drop_last=True)

    crit = Critic()
    gen = Generator()

    # gen.load_state_dict(torch.load("models/w_generator2_18"))
    # crit.load_state_dict(torch.load("models/critic2_18"))
    ...
    c_losses, g_losses = train_wgan(crit, gen, train_dataloader, epochs, 1e-4)
