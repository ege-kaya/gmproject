...
def train_wgan(critic, generator, train_dataloader, epochs, step_size):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 5 == 4:
            torch.save(crit.state_dict(), f"models/ncritic_{epoch+1}")
            torch.save(gen.state_dict(), f"models/nw_generator_{epoch+1}")
        
            with open(f"losses/c_losses_{epoch+1}", "wb") as fp:
                pickle.dump(c_losses, fp)

            with open(f"losses/w_g_losses_{epoch+1}", "wb") as fp:
